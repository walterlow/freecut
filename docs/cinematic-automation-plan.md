...
## Director 4K Interface And Source-Diversity Gate

The automatic high-end workflow is now exposed directly in the editor at `Studio > Cinematic Story Edit`. The `Director 4K` production-mode button configures the full stack together: Magnates 3D documentary grammar, native 4K Pixabay coverage, transcript matching, staged multi-axis camera movement, depth preparation, sparse directed transitions, lifted cinematic finishing, automatic score, imported-library preference, licensed Freesound layers, and stronger-take auditioning. The mode reports `Active` only while every required production layer remains enabled.

Timeline audit now measures visual-source diversity separately from shot count. It reports distinct sources versus coverage shots and penalizes sequences that inflate editorial density by repeatedly cropping the same few assets. This closes the main weakness in the latest proof film, which cut fourteen times but relied on only six source images. A sequence with strong motion, parallax, subtitles, and audio can no longer receive an inflated reference-readiness score while its coverage still looks repetitive.
