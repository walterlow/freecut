...
export interface ResolvedTransitionWindow<
  TClip extends TimelineClipLike = TimelineClipLike,
  TTransition extends TransitionLike = TransitionLike,
> {
...
interface MutableResolvedTransition<
  TClip extends TimelineClipLike,
  TTransition extends TransitionLike,
> {
  transition: TTransition;
  leftClip: TClip;
  rightClip: TClip;
...
>(
  ...
  clipsById: ReadonlyMap<string, TClip>,
): ResolvedTransitionWindow<TClip, TTransition>[] {
  const resolvedByTransitionId = new Map<string, MutableResolvedTransition<TClip, TTransition>>();
  ...
  const windows: Array<ResolvedTransitionWindow<TClip, TTransition>> = [];
