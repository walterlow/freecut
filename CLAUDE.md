...
- Transition renderers in `domain/timeline/transitions/renderers/` have dual render paths: `calculateStyles()` (CSS for preview) and `renderCanvas()` (canvas for export). Changes must be applied to **both** or preview/export will diverge. Canvas `drawImage` offsets must use `Math.round()` to avoid sub-pixel interpolation artifacts; CSS transforms keep sub-pixel precision (GPU compositor handles them smoothly)
