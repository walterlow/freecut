...
describe('gpu-curves', () => {
  ...
  it('converts legacy master params into explicit control points', () => {
    ...
    expect(evaluateGpuCurvesEffectChannel(legacyParams, 'master', 0.25)).toBeCloseTo(control.shadow.y, 4);
    expect(evaluateGpuCurvesEffectChannel(legacyParams, 'master', 0.75)).toBeCloseTo(control.highlight.y, 4);
  });
