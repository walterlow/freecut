...
describe('text-layout-drafts', () => {
  it('detects text layout mode from span count', () => {
    ...
    expect(getTextItemLayoutMode(createTextItem({
  ...
  it('builds editable base spans from single text styling', () => {
    const spans = buildEditableBaseSpans(createTextItem({
      text: 'Solo',
      fontSize: 88,
  ...
  it('derives single draft from the title span in three-span layouts', () => {
    const draft = buildTextSingleLayoutDraft(createTextItem({
      text: 'Eyebrow\nHeadline\nSubtitle',
      fontSize: 60,
      fontFamily: 'Inter',
      color: '#ffffff',
      textSpans: [
        ...
        { text: 'Headline', fontSize: 92, fontFamily: 'Bebas Neue', fontWeight: 'bold', color: '#f8fafc' },
