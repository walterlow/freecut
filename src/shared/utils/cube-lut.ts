...
function sampleTrilinear(
  ...
  const outG0 = lerp(
    lerp(table[c000 + 1]!, table[c100 + 1]!, tr),
    lerp(table[c010 + 1]!, table[c110 + 1]!, tr),
    tg,
  )
  const outG1 = lerp(
    lerp(table[c001 + 1]!, table[c101 + 1]!, tr),
    lerp(table[c011 + 1]!, table[c111 + 1]!, tr),
    tg,
  )

  const outB0 = lerp(
    lerp(table[c000 + 2]!, table[c100 + 2]!, tr),
    lerp(table[c010 + 2]!, table[c110 + 2]!, tr),
    tg,
  )
  const outB1 = lerp(
    lerp(table[c001 + 2]!, table[c101 + 2]!, tr),
    lerp(table[c011 + 2]!, table[c111 + 2]!, tr),
    tg,
  )
  ...
  apply(
    ...
    try {
      ...
      gl.texImage2D(
        ...
        gl.RGBA,
        gl.UNSIGNED_BYTE,
        sourceCanvas as TexImageSource,
      )
