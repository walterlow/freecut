...
function bilinear(
  ...
  const top: [number, number] = [tl[0] + (tr[0] - tl[0]) * s, tl[1] + (tr[1] - tl[1]) * s]
  const bot: [number, number] = [bl[0] + (br[0] - bl[0]) * s, bl[1] + (br[1] - bl[1]) * s]
...
export function generateCornerPinMesh(
  ...
  function writeVertex(s: number, t: number): void {
    const [ox, oy] = bilinear(
      outputCorners.topLeft,
      outputCorners.topRight,
      outputCorners.bottomRight,
      outputCorners.bottomLeft,
      s,
      t,
    )
    const [cx, cy] = toClipSpace(ox, oy)
    const [uvx, uvy] = bilinear(
      inputCorners.topLeft,
      inputCorners.topRight,
      inputCorners.bottomRight,
      inputCorners.bottomLeft,
      s,
      t,
    )
    data[offset++] = cx
...
export function isIdentityCornerPin(corners: CornerPinCorners): boolean {
  const eps = 0.001
  return (
    Math.abs(corners.topLeft[0]) < eps &&
    Math.abs(corners.topLeft[1]) < eps &&
    Math.abs(corners.topRight[0] - 1) < eps &&
    Math.abs(corners.topRight[1]) < eps &&
    Math.abs(corners.bottomRight[0] - 1) < eps &&
    Math.abs(corners.bottomRight[1] - 1) < eps &&
    Math.abs(corners.bottomLeft[0]) < eps &&
    Math.abs(corners.bottomLeft[1] - 1) < eps
  )
}
