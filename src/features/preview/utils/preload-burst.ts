...
export function getPreloadBurstTrigger(input: {
  ...
  if (input.interactionMode === 'scrubbing' && input.prevInteractionMode !== 'scrubbing') {
    return 'scrub_enter'
  }
  ...
  if (
    ...
    isShortSeekFrameDelta(Math.abs(input.currentFrame - input.prevCurrentFrame), input.fps)
  ) {
