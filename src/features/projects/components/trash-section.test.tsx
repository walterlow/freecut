...
vi.mock('@/infrastructure/storage', () => ({
  listTrashedProjects: (...args: unknown[]) =>
    listTrashedProjectsMock(...(args as [])),
}));
...
describe('TrashSection', () => {
  ...
  it('empties the entire trash in one confirm', async () => {
    ...
    expect(
      await screen.findByText(/permanently delete 2 project\(s\)/i),
    ).toBeInTheDocument();
    ...
    await waitFor(() => {
      expect(toastSuccess).toHaveBeenCalledWith(
        'Emptied trash (2 projects deleted)',
      );
    });
  });
