...
export function EqOutputGainControl({
  ...
  const thumbPercent =
    (1 - (displayValue - AUDIO_EQ_GAIN_DB_MIN) / Math.max(range, 1)) * 100;

  const valueFromClientY = useCallback(
    (clientY: number, rect: DOMRect) => {
      const normalized = 1 - (clientY - rect.top) / Math.max(rect.height, 1);
      return roundOutputGainDb(
        AUDIO_EQ_GAIN_DB_MIN + Math.max(0, Math.min(1, normalized)) * range,
      );
    },
    [range],
  );
      ...
      >
        <div className="absolute inset-x-0 top-0 bottom-7">
          {[20, 10, 0, -10, -20].map((tick) => {
            const tickPercent =
              (1 - (tick - AUDIO_EQ_GAIN_DB_MIN) / Math.max(range, 1)) * 100;
            return (
              <div
                key={tick}
