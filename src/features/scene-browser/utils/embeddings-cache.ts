...
import { pairCaptionsWithMotion } from './pair-motion';
...
export function indexMediaCaptions(mediaId: string): Promise<void> {
  ...
  const promise = (async () => {
    ...
    const [transcript, scenes] = await Promise.all([
      getTranscript(mediaId).catch(() => null),
      getScenes(mediaId).catch(() => null),
    ]);
    const motionByIndex = pairCaptionsWithMotion(
      media.aiCaptions.map((c) => ({ timeSec: c.timeSec })),
      scenes?.cuts,
    );
    ...
    // Stamp the extracted palettes + inferred motion onto each caption
    // so retroactive indexing also populates color + motion data for
    // legacy captions without them.
